...
// ── In-Memory Cache ──────────────────────────────────────────────
// Simple TTL cache for API responses to avoid hitting external APIs on every request.
...
function cacheSet(key, data, ttlMs) {
    // Evict oldest entries when the cache grows past the cap (FIFO).
...
let priceHistoryDirty = false;
let priceHistoryTimer = null;

function loadPriceHistorySync() {
    try {
...
function getPriceHistory() {
    if (!priceHistoryCache) {
        priceHistoryCache = loadPriceHistorySync();
    }
    return priceHistoryCache;
}

function flushPriceHistory() {
    if (!priceHistoryDirty) return;
    priceHistoryDirty = false;
    try {
        if (!fs.existsSync(PRICE_HISTORY_DIR)) {
            fs.mkdirSync(PRICE_HISTORY_DIR, { recursive: true });
        }
        fs.writeFileSync(PRICE_HISTORY_FILE, JSON.stringify(priceHistoryCache, null, 2), "utf-8");
    } catch (e) {
        console.error("Price history flush error:", e.message);
    }
}

// Flush dirty price history to disk every 5 seconds instead of on every write.
priceHistoryTimer = setInterval(flushPriceHistory, 5000);

process.on("SIGINT", () => { flushPriceHistory(); clearInterval(priceHistoryTimer); process.exit(); });
process.on("SIGTERM", () => { flushPriceHistory(); clearInterval(priceHistoryTimer); process.exit(); });
...
function cacheStockOHLCV(symbol, ohlcv) {
    ...
    priceHistoryDirty = true;
}

// ── Axios defaults ───────────────────────────────────────────────
const NEPSE_API = axios.create({
...
app.use(
    express.static(path.join(__dirname, "public"), {
        ...
        setHeaders(res, filePath) {
            ...
            // Cache versioned CSS/JS aggressively
...
// ── NEPSE Proxy APIs (with cache + timeouts) ─────────────────────
// Cache durations: market data changes during trading hours, cache for 30s-2min
const NEPSE_CACHE_TTL = 30000;   // 30 seconds for live index
const NEPSE_MOVERS_TTL = 60000;  // 1 minute for gainers/losers/turnover
const NEPSE_STATIC_TTL = 120000; // 2 minutes for sub-indices, company list

app.get("/api/nepse-index", async (req, res) => {
    try {
        const data = await cachedFetch("nepse-index", NEPSE_CACHE_TTL, async () => {
            const response = await NEPSE_API.get("/NepseIndex");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("NEPSE index error:", error.message);
        const stale = cacheGet("nepse-index");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch NEPSE data" });
    }
});

app.get("/api/top-gainers", async (req, res) => {
    try {
        const data = await cachedFetch("top-gainers", NEPSE_MOVERS_TTL, async () => {
            const response = await NEPSE_API.get("/TopGainers");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Top gainers error:", error.message);
        const stale = cacheGet("top-gainers");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch top gainers" });
    }
});

app.get("/api/top-losers", async (req, res) => {
    try {
        const data = await cachedFetch("top-losers", NEPSE_MOVERS_TTL, async () => {
            const response = await NEPSE_API.get("/TopLosers");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Top losers error:", error.message);
        const stale = cacheGet("top-losers");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch top losers" });
    }
});

app.get("/api/CompanyList", async (req, res) => {
    try {
        const data = await cachedFetch("company-list", NEPSE_STATIC_TTL, async () => {
            const response = await NEPSE_API.get("/CompanyList");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Company list error:", error.message);
        const stale = cacheGet("company-list");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch company list" });
    }
});
...
app.get("/api/CompanyDetails/:symbol", async (req, res) => {
    try {
        ...
        const cacheKey = "company-details:" + symbol;

        const data = await cachedFetch(cacheKey, NEPSE_CACHE_TTL, async () => {
            const response = await NEPSE_API.get("/CompanyDetails", { params: { symbol } });
            return response.data;
        });
        ...
        if (businessDate && dto.lastTradedPrice) {
            ...
            try { cacheStockOHLCV(symbol, ohlcv); } catch (e) { /* ignore */ }
        }

        res.json(data);
    } catch (error) {
        console.error("Company details error:", error.message);
        res.status(500).json({ error: "Unable to fetch company details" });
    }
});

app.get("/api/TopTenTurnoverScrips", async (req, res) => {
    try {
        const data = await cachedFetch("top-ten-turnover", NEPSE_MOVERS_TTL, async () => {
            const response = await NEPSE_API.get("/TopTenTurnoverScrips");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Top turnover error:", error.message);
        const stale = cacheGet("top-ten-turnover");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch turnover data" });
    }
});

app.get("/api/TradeTurnoverTransactionSubindices", async (req, res) => {
    try {
        const data = await cachedFetch("trade-turnover-sub", NEPSE_MOVERS_TTL, async () => {
            const response = await NEPSE_API.get("/TradeTurnoverTransactionSubindices");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Trade turnover sub error:", error.message);
        const stale = cacheGet("trade-turnover-sub");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch trade data" });
    }
});

app.get("/api/TopTenTransactionScrips", async (req, res) => {
    try {
        const data = await cachedFetch("top-ten-transaction", NEPSE_MOVERS_TTL, async () => {
            const response = await NEPSE_API.get("/TopTenTransactionScrips");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Top transaction error:", error.message);
        const stale = cacheGet("top-ten-transaction");
        if (stale) return res.json(stale);
...
app.get("/api/NepseSubIndices", async (req, res) => {
    try {
        const data = await cachedFetch("nepse-sub-indices", NEPSE_STATIC_TTL, async () => {
            const response = await NEPSE_API.get("/NepseSubIndices");
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("Sub indices error:", error.message);
        const stale = cacheGet("nepse-sub-indices");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch sub-indices" });
    }
});
...
app.get("/api/DailyScripPriceGraph", async (req, res) => {
    try {
        ...
        const data = await cachedFetch("price-graph:" + symbol, NEPSE_CACHE_TTL, async () => {
            const response = await NEPSE_API.get("/DailyScripPriceGraph", {
                params: { symbol, type: 1 },
                timeout: 15000
            });
            return response.data;
        });
        ...
        console.error("Price graph error:", error.message);
        res.status(500).json({ error: "Unable to fetch price graph" });
    }
});
...
app.get("/api/StockHistory/:symbol", async (req, res) => {
    try {
        ...
        const data = await cachedFetch("stock-history:" + symbol, 60000, async () => {
            const [histRes, intraRes] = await Promise.all([
                NEPSE_API.get("/PriceVolumeHistory", {
                    params: { symbol, size: 250 },
                    timeout: 15000
                }).catch(() => ({ data: [] })),
                NEPSE_API.get("/DailyScripPriceGraph", {
                    params: { symbol, type: 1 },
                    timeout: 15000
                }).catch(() => ({ data: [] }))
            ]);
            ...
            return { daily: dailyCandles, intraday: intraCandles };
        });
...
app.get("/api/DailyNepseIndexGraph", async (req, res) => {
    try {
        const data = await cachedFetch("nepse-index-graph", NEPSE_STATIC_TTL, async () => {
            const response = await NEPSE_API.get("/DailyNepseIndexGraph", { timeout: 15000 });
            return response.data;
        });
        res.json(data);
    } catch (error) {
        console.error("NEPSE index graph error:", error.message);
        const stale = cacheGet("nepse-index-graph");
        if (stale) return res.json(stale);
        res.status(500).json({ error: "Unable to fetch NEPSE index graph" });
    }
});
...
// ── Commodities API (PARALLEL fetch + background refresh) ────────
// Previously fetched 13 quotes SEQUENTIALLY (13-65 seconds!).
// Now fetches all in parallel and caches the result.
...
const COMMODITIES_CACHE_TTL = 300000; // 5 minutes

async function refreshCommodities() {
    try {
        const results = await Promise.allSettled(
            COMMODITY_SYMBOLS.map(item =>
                yahooFinance.quote(item.symbol).then(quote => ({
                    symbol: item.symbol,
                    name: item.name,
                    ...
                    console.error(`Commodity quote error ${item.symbol}:`, e.message);
                    return { symbol: item.symbol, name: item.name, price: 0, change: 0, changePercent: 0, error: true };
                })
            )
        );

        const data = results.map(r => r.status === "fulfilled" ? r.value : { symbol: "?", name: "?", price: 0, error: true });
        cacheSet("commodities", data, COMMODITIES_CACHE_TTL);
        console.log("[Commodities] Background refresh complete");
    } catch (err) {
        console.error("[Commodities] Background refresh failed:", err.message);
    }
}

// Initial fetch + periodic refresh every 5 minutes
refreshCommodities();
setInterval(refreshCommodities, COMMODITIES_CACHE_TTL);
...
app.get("/api/commodities", async (req, res) => {
    try {
        ...
        // If no cache yet, fetch in parallel (not sequentially!)
        const results = await Promise.allSettled(
            COMMODITY_SYMBOLS.map(item =>
                yahooFinance.quote(item.symbol).then(quote => ({
                    symbol: item.symbol,
                    ...
                    symbol: item.symbol, name: item.name, price: 0, change: 0, changePercent: 0, error: true
                }))
            )
        );
...
app.get("/api/crypto", async (req, res) => {
    try {
        const data = await cachedFetch("crypto", CRYPTO_CACHE_TTL, async () => {
...
app.get("/api/crypto/candle", async (req, res) => {
    try {
        ...
        const cacheKey = `crypto-candle:${symbol}:${interval}:${limit}`;
        const data = await cachedFetch(cacheKey, 60000, async () => {
            const response = await axios.get(
                "https://api.binance.com/api/v3/klines",
                { params: { symbol, interval, limit }, timeout: 10000 }
            );
...
app.get("/api/commodity/candle", async (req, res) => {
    try {
        ...
        const cacheKey = `commodity-candle:${symbol}:${range}`;

        const data = await cachedFetch(cacheKey, range === "1d" ? 300000 : 3600000, async () => {
            ...
            if (!response.quotes || response.quotes.length === 0) return [];
...
app.get("/stock/:symbol", async (req, res) => {
    try {
        ...
        res.render("stock", { symbol: req.params.symbol, user });
    } catch (error) {
...
// ── Global Prices ────────────────────────────────────────────────
const GLOBAL_PRICES_CACHE_TTL = 30000;

app.get("/api/global-prices", async (req, res) => {
    try {
        const data = await cachedFetch("global-prices", GLOBAL_PRICES_CACHE_TTL, async () => {
            ...
            return {
                usdNpr: usdNpr,
                btc: { usd: btcUsd, npr: btcUsd * usdNpr, change: Number(btcRes.data.priceChangePercent || 0) },
                eth: { usd: ethUsd, npr: ethUsd * usdNpr, change: Number(fxRes.data.priceChangePercent || 0) },
        ...
        const stale = cacheGet("global-prices");
        if (stale) return res.json(stale);
